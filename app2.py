...
if uploaded_pdf:
    ...
    # --- PDF -> Image (alle Seiten) ---
    pages = convert_from_bytes(uploaded_pdf.read(), dpi=dpi)
    page_num = st.number_input("Seitenzahl wählen", min_value=1, max_value=len(pages), value=1)
    plan_image = pages[page_num - 1]
